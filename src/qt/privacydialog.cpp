...
#include "zgreencoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zGRNC ought to be enough for anybody." - Bill Gates, 2017
    ui->zGRNCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzGRNCSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzGRNC->setEnabled(false);
        ui->pushButtonMintzGRNC->setToolTip(tr("zGRNC is currently disabled due to maintenance."));

        ui->pushButtonSpendzGRNC->setEnabled(false);
        ui->pushButtonSpendzGRNC->setToolTip(tr("zGRNC is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zGRNCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzGRNC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGRNC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zGRNC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zGRNC in ") + 
...
void PrivacyDialog::on_pushButtonSpendzGRNC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGRNC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zGRNC
        sendzGRNC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zGRNC
    sendzGRNC();
}

void PrivacyDialog::on_pushButtonZGreenCoinControl_clicked()
{
    ZGreenCoinControlDialog* zGreenCoinControl = new ZGreenCoinControlDialog(this);
    zGreenCoinControl->setModel(walletModel);
    zGreenCoinControl->exec();
}

void PrivacyDialog::setZGreenCoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzGreenCoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzGRNC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid GreenCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zGRNCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zGRNC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " GRNC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zGRNC</b>";
    ...
    // use mints from zGreenCoin selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZGreenCoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZGreenCoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zgreencoin selector in case it was used
    ZGreenCoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zGreenCoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " GreenCoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zGreenCoin Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zGRNC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zGRNC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zGRNC "));
    ui->labelzGRNCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzGRNCSyncStatus->setVisible(fShow);
}
