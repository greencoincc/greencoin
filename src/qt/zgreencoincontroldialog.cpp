...
#include "ui_zgreencoincontroldialog.h"
...
std::list<std::string> ZGreenCoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZGreenCoinControlDialog::listMints;

ZGreenCoinControlDialog::ZGreenCoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZGreenCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZGreenCoinControlDialog::~ZGreenCoinControlDialog()
{
    delete ui;
}

void ZGreenCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGreenCoinControlDialog::updateList()
{
...
void ZGreenCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGreenCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZGreenCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGreenCoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZGreenCoinControlDialog::GetSelectedMints()
{
...
void ZGreenCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
